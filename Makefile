BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Das_Leben_des_Feldmarschalls_Grafen_York_von_Wartenburg"
TITLE_NAME = "Das Leben des Feldmarschalls Grafen York von Wartenburg"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Zum_Kontinent_des_eisigen_Suedens_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Zum_Kontinent_des_eisigen_Suedens_*.jpg
	rm cover.jpg 
	rm logo.jpg
